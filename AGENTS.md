...
- **CRÍTICO — `/api/suscripcion/cancel` no cancela el preapproval en MercadoPago**: hoy solo hace `prisma.suscripcion.update(... estado: "CANCELADA")`, pero nunca llama a la API de MercadoPago para cancelar el preapproval real (`new PreApproval(mpClient()).update({ id: mercadopagoPreapprovalId, body: { status: "cancelled" } })`, método `update` ya disponible en la SDK instalada). Sin este fix, un profesional que cancela en fixhub sigue siendo cobrado automáticamente todos los meses por MercadoPago. Hay que arreglarlo antes de cobrar a usuarios reales.

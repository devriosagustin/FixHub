...
- **`MERCADOPAGO_WEBHOOK_SECRET` falta en `docker-compose.yml` y `.env.docker`**: el código ya valida la firma del webhook (`validarFirmaWebhook`), pero la variable no está en el `environment:` de `docker-compose.yml` ni documentada en `.env.docker` — agregarla en ambos y cargar el secret real al desplegar.

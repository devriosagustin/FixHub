...
- **`verificar-vencidas` no tiene scheduler**: es idempotente y pensado para cron, pero `docker-compose.yml` no define ningún cron/scheduler. Si se lo sigue necesitando (vencimientos sin preapproval real), hay que programarlo externamente (cron del servidor, GitHub Action, etc.) al desplegar.
...
### Base de datos
- Hoy solo se usa `prisma db push` (`db:push` en `package.json`), no hay carpeta `prisma/migrations` — no hay historial de migraciones versionado ni forma de hacer rollback de un cambio de esquema. Antes de producción conviene pasar a `prisma migrate dev` (para generar el baseline) y `prisma migrate deploy` en el arranque/CI en vez de `db push`, que además puede aplicar cambios destructivos sin avisar en modo no interactivo.

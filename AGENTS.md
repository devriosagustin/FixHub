...
### Base de datos
- ~~Hoy solo se usa `prisma db push` (`db:push` en `package.json`), no hay carpeta `prisma/migrations`~~ — **en progreso 2026-09-07**: se agregaron los scripts `db:migrate` (`prisma migrate dev`) y `db:migrate:deploy` (`prisma migrate deploy`) a `package.json`. Falta el paso de "baseline" (generar la migración inicial a partir del estado actual de la base, ya creada con `db push`, sin volver a ejecutar el SQL). No se pudo generar desde el sandbox de este agente porque el motor de Prisma para Linux no está cacheado ahí y no hay acceso de red para descargarlo (403 al pedir `binaries.prisma.sh`); hay que correrlo a mano en una terminal con acceso a la base real (la del desarrollo local alcanza):
  ```
  ...
  ```
  El segundo comando marca esa migración como ya aplicada (no la vuelve a ejecutar), porque la base ya tiene ese esquema por `db push`. Después de esto, commitear la carpeta `prisma/migrations/` y pasar a usar `db:migrate` (dev) / `db:migrate:deploy` (prod/CI) en vez de `db:push` para los próximos cambios de esquema, que además puede aplicar cambios destructivos sin avisar en modo no interactivo.
...
- `DB_PASSWORD` en `.env.docker` tiene como default `changeme` (y `docker-compose.yml` cae a ese default si no está seteada) — cambiarla por una contraseña real antes de desplegar.
